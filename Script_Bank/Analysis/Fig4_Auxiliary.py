...
_initiate_set_values = [ # [0, 25, 50, 75, 80, 85, 90, 95, 100, 105, 110, 115, 120, 125, 150, 175, 200, 225, 250]
    [[0]*14, [0, 25, 50, 75, 100, 125, 150, 175, 200, 225, 250]], # FC_MRNA
    [[0, 25, 50, 75, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000], [0, 25, 50, 75, 100, 125, 150, 175, 200, 225, 250]] # FC
]
        ...
        for initiate in initiate_set:
            ...
            for clue in clues:
                ...
                for spa in species_selection:
                    ...
                    y = np.ma.masked_array(data = y_cellular_stats, mask = np.logical_not(y_classifier))
                    if synopses['Mean']:
                    ...
                    axe[row, col].set_title(label = f'{clue_labels[clue_index]}\nμ ± σ', fontsize = font_size_alp)
                    axe[row, col].set_xticks(ticks = _x, labels = [_ if _ % 8 == 0 else None for _ in _x], fontsize = font_size_alp, rotation = 0)
                    axe[row, col].set_yticks(ticks = _y, labels = [_ if _ % 250 == 0 else None for _ in _y], fontsize = font_size_alp, rotation = 0) if col == 0 else axe[row, col].set_yticks(ticks = _y, labels = [], fontsize = font_size_alp, rotation = 0)
                    if col != cols: axe[row, col].set_xlabel('Time [Hour]', fontsize = font_size_alp)
                    if col == 0: axe[row, col].set_ylabel('Molecule Count', fontsize = font_size_alp)
                    axe[row, col].set_xlim(x_limes[0], x_limes[1]) # x_limes
                    axe[row, col].set_ylim(y_limes[0], y_limes[1]) # y_limes
                    if col == 0: axe[row, col].legend(fontsize = font_size_bet, loc = 'upper right')
# plt.show()
    ...
    for cellulate in cellulates:
        ...
        for initiate in initiate_set:
            ...
            for clue in clues:
                ...
                axe[row, col].set_title(label = 'Time = 48 Hours', fontsize = font_size_alp)
                axe[row, col].set_xticks(ticks = _x, labels = [_ if _ % 250 == 0 else None for _ in _x], fontsize = font_size_alp, rotation = 0)
                axe[row, col].set_yticks(ticks = _y, labels = [_ if _ % 250 == 0 else None for _ in _y], fontsize = font_size_alp, rotation = 0)
                axe[row, col].set_xlabel(species_selection_labels[0], fontsize = font_size_alp)
                axe[row, col].set_ylabel(species_selection_labels[1], fontsize = font_size_alp)
                axe[row, col].set_xlim(x_limes[0], x_limes[1]) # x_limes
                axe[row, col].set_ylim(y_limes[0], y_limes[1]) # y_limes
                axe[row, col].legend(fontsize = font_size_bet, loc = 'upper right')
                row = 0
                col = 1
                ...
                axe[row, col].set_title(label = 'Time = 48 Hours', fontsize = font_size_alp)
                axe[row, col].set_xticks(ticks = _x, labels = [_ if _ % 250 == 0 else None for _ in _x], fontsize = font_size_alp, rotation = 0)
                axe[row, col].set_yticks(ticks = _z, labels = [_ if _ % 50 == 0 else None for _ in _z], fontsize = font_size_alp, rotation = 0)
                axe[row, col].set_xlabel(species_selection_labels[0], fontsize = font_size_alp)
                axe[row, col].set_ylabel(species_selection_labels[2], fontsize = font_size_alp)
                axe[row, col].set_xlim(x_limes[0], x_limes[1]) # x_limes
                axe[row, col].set_ylim(z_limes[0], z_limes[1]) # z_limes
                # axe[row, col].legend(fontsize = font_size_bet)
                row = 0
                col = 2
                ...
                axe[row, col].set_title(label = tits[col], loc = 'left', fontsize = font_size_alp, fontweight = 'bold')
                axe[row, col].set_title(label = 'Time = 48 Hours', fontsize = font_size_alp)
                axe[row, col].set_xticks(ticks = _z, labels = [_ if _ % 50 == 0 else None for _ in _z], fontsize = font_size_alp, rotation = 0)
                axe[row, col].set_yticks(ticks = _y, labels = [_ if _ % 250 == 0 else None for _ in _y], fontsize = font_size_alp, rotation = 0)
                axe[row, col].set_xlabel(species_selection_labels[2], fontsize = font_size_alp)
                axe[row, col].set_ylabel(species_selection_labels[1], fontsize = font_size_alp)
            ...
            _axe_h[0, 0].set_xticks(ticks = _x, labels = [], fontsize = font_size_bet, rotation = 0)
            _axe_h[0, 0].set_yticks(ticks = [], labels = [], fontsize = font_size_bet, rotation = 0)
            ...
            _axe_v[0, 0].set_xticks(ticks = [], labels = [], fontsize = font_size_bet, rotation = 0)
            _axe_v[0, 0].set_yticks(ticks = _y, labels = [], fontsize = font_size_bet, rotation = 0)
            ...
            _axe_h[0, 1].set_xticks(ticks = _x, labels = [], fontsize = font_size_bet, rotation = 0)
            _axe_h[0, 1].set_yticks(ticks = [], labels = [], fontsize = font_size_bet, rotation = 0)
            ...
            _axe_v[0, 1].set_xticks(ticks = [], labels = [], fontsize = font_size_bet, rotation = 0)
            _axe_v[0, 1].set_yticks(ticks = _z, labels = [], fontsize = font_size_bet, rotation = 0)
            ...
            _axe_h[0, 2].set_xticks(ticks = _z, labels = [], fontsize = font_size_bet, rotation = 0)
            _axe_h[0, 2].set_yticks(ticks = [], labels = [], fontsize = font_size_bet, rotation = 0)
            ...
            _axe_v[0, 2].set_xticks(ticks = [], labels = [], fontsize = font_size_bet, rotation = 0)
            _axe_v[0, 2].set_yticks(ticks = _y, labels = [], fontsize = font_size_bet, rotation = 0)
