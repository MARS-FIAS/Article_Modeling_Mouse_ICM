...
row = 0
_x = [f'{_[0]-100}%' for _ in initiate_set]
_y = cards
tits = ['[B]', '[C]', '[D]']
axe = axe_mat[0:1, 1:4]
for col in range(cols):
...
for cellulate in cellulates:
    ...
    for initiate in initiate_set:
        ...
        for species_selection in _species_selection:
            ...
            axe[row, col].scatter(x, y, s = mark_size, c = coco, marker = mark, edgecolors = 'none', alpha = 0.75, label = f'{initiate[0]-100}%')
            ...
            axe[row, col].set_title(label = '[A]', loc = 'left', fontsize = font_size_alp, fontweight = 'bold', position = (-0.25, 1))
