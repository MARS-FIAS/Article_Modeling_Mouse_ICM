...
auto_para_mem = (0, 0, 0) # (chi_auto, chi_para, ksig_M)
_auto_para_mem = '_'.join(map(str, auto_para_mem))
reiterate = f'Reiterate_{cellulate[0]}_{cellulate[1]}_Auto_Para_Mem_{_auto_para_mem}'
...
print(f"Reiterate!\n{' '*8}{reiterate}")
auto, para, mem = auto_para_mem
service = [pow(2, len(auto_para_mem)-1-_)*auto_para_mem[_] for _ in range(len(auto_para_mem))]
print(f"Service!\n{' '*8}{sum(service)}\n{' '*2*8}{auto_para_mem}\n{' '*2*8}{service}")
mape_temp = posterior.map().numpy()
_mape = np.concatenate((mape_temp, 1-mape_temp[[len(mape_temp)-1]]))
...
extrema = [(0, (0, 0+span)), (1, (1-span, 1))]
ksig_M = 1/mape[13]
doze[13] = (ksig_M, (ksig_M*(1-span), ksig_M*(1+span))) # tau_M ---->>>> ksig_M
...
para_set_raw = {
    ...
    'tau_C': doze[12], 'ksig_M': doze[13] if mem else extrema[0],
    'tau_ef_EA': doze[14], 'tau_eb_EA': doze[15], 'tau_pf_NP': doze[16], 'tau_pb_NP': doze[17],
    'chi_auto': extrema[1] if auto and not para else extrema[0] if not auto or not para else doze[18],
    'chi_para': extrema[1] if para and not auto else extrema[0] if not para or not auto else doze[19]
}
para_set_mode = 0 # {0, 1}
para_set, para_set_true = make_paras(para_set_raw = para_set_raw, para_set_mode = para_set_mode, verbose = True)
team_temp = ['chi_auto', 'chi_para', 'ksig_M']
...
def simulator(parameter_set, parameter_set_true, parameter_set_mode, parameter_set_rules = None, species = ['N', 'G', 'N_MRNA', 'G_MRNA', 'NP', 'FC', 'FC_MRNA', 'FM', 'FD', 'EI', 'EA'], steps = 10000, cells = 3, time_mini = 0, time_maxi = 48, time_unit = 'Hours', time_delta = 0.2, cell_layers = 2, layer_cells = 1, faces = 2*3, seed = None, sup_comp = False, verbose = False):
    ...
    for rule in parameter_set_rules:
        ...
        ksig_M = rule*para_fun('ksig_M')
        chi_auto = np.round(para_fun('chi_auto'), 2)
        chi_para = np.round(para_fun('chi_para'), 2)
        rate_mat = instate_rate_mat(stem, cells, parameter_set, parameter_set_true, para_fun, rates_exclude, rho_mat, blank = False, kd_FC = kd_FC, kd_FM = kd_FM, ksig_C = ksig_C, ksig_M = ksig_M, chi_auto = chi_auto, chi_para = chi_para)
...
parameter_set_rules = [1] if any(auto_para_mem) else [0] # None
