...
wait = 0 # Hours
reiterate = f'Reiterate_Nook_{cellulate[0]}_{cellulate[1]}_Initiate_{_initiate}_Wait_{wait}'
...
def simulator(parameter_set, parameter_set_true, parameter_set_mode, parameter_set_rules = None, species = ['N', 'G', 'N_MRNA', 'G_MRNA', 'NP', 'FC', 'FC_MRNA', 'FM', 'FD', 'EI', 'EA'], steps = 10000, cells = 3, time_mini = 0, time_wait = 0, time_maxi = 48, time_unit = 'Hours', time_delta = 0.25, cell_layers = 2, layer_cells = 1, faces = 2*3, seed = None, sup_comp = False, verbose = False):
    ...
    for rule in parameter_set_rules:
        ...
        if verbose: plotful(data_inter_wait, species, time_unit)
        ...
        state_tors = {'state_tor_PRE': _state_tor, 'state_tor_EPI': _state_tor}
        state_tor = instate_state_tor(stem, cell_layers, layer_cells, state_tors, pat_mode, initial_pattern, blank = False, state_tor_wait = simul.state_tor[0])
