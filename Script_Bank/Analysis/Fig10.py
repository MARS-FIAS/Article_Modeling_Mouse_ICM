...
def retrieve_data_memo_initiate(memo, cellulates, initiate_set, wait, verbose = False, _reiteration = 'Reiterate'):
    ...
    for cellulate in cellulates:
        for initiate_temp in initiate_set_temp:
            ...
            reiteration = _reiteration+'_'+f'{cellulate[0]}_{cellulate[1]}_Initiate_{_initiate}_Wait_{wait}' # {Wild, Nook, Cast}
...
_exam = [0, 25, 50, 75, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000] # [0, 250, 500, 750, 1000]
_fate = ['NT', 'G', 'D']
_stat = ['mean', 'sand']
...
for col in range(cols):
    ...
    axe[row, col].set_yticks(ticks = _exam, labels = [int(_/2) if _ % 50 == 0 else None for _ in _exam], fontsize = font_size_bet) if col == 0 else axe[row, col].set_yticks(ticks = _exam, labels = [], fontsize = font_size_bet)
...
for wait in _wait:
    ...
    initiate_set_index = data_paths.index(_memo)
    initiate_set_temp = _initiate_set[initiate_set_index]
    data_memo_initiate = retrieve_data_memo_initiate(memo, cellulates, initiate_set_temp, wait, verbose = True, _reiteration = _reiteration)
    for cellulate in cellulates:
        cellulate_index = cellulates.index(cellulate)
        cellulate_cells = cells[cellulate_index]
        normalization = np.power(cellulate_cells, normalize)
...
for cellulate in cellulates:
    ...
    for initiate in initiate_set:
        ...
        for row in range(rows):
            ...
            tag = [int(_*mapes[i][13]/100) for _ in _initiate_set[i]['FC']] if percentage[i] else [int(_/2) for _ in _initiate_set[i]['FC']]
            axe[row, col].set_xticks(ticks = range(len(initiate_set)), labels = tag, fontsize = font_size_bet, rotation = 45)
