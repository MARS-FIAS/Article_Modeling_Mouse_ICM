...
initiate = {'FC_MRNA': 0, 'FC': 0} # {0, ..., 250} # {0, 25, 50, 75, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000} # Toll!
_initiate = '_'.join(map(str, [f'{key}_{value}' for key, value in initiate.items()]))
wait = 0 # Hours
reiterate = f'Reiterate_Cast_{cellulate[0]}_{cellulate[1]}_Initiate_{_initiate}_Wait_{wait}'
...
if sup_comp:
    ...
    path = os.path.dirname(os.path.realpath(__file__)) + f'/../../Data_Bank/{data_path}/' + _path
post = '_Posterior.pkl'
...
print(f"Reiterate!\n{' '*8}{reiterate}")
service = f"\n{' '*8}".join(map(str, [f"{key}\n{' '*2*8}{value}" for key, value in initiate.items()]))
print(f"Service!\n{' '*8}{service}")
mape_temp = posterior.map().numpy()
_mape = np.concatenate((mape_temp, list(initiate.values())))
mape = [int(round(_mape[index], 0)) if _mape[index] > 1 else round(_mape[index], 2) for index in range(_mape.size)]
print(f'Map!\n{_tag}\n\t{mape}')
spot = 2
mape[spot] *= 1
span = 5e-31
doze = [(_mape, (round(_mape*(1-span), 31), round(_mape*(1+span), 31))) for _mape in mape]

para_set_raw = {
    'N_N': doze[0], 'G_G': doze[1], 'FC_N': doze[2], 'G_EA': doze[3],
    'G_N': doze[4], 'N_G': doze[5], 'FC_G': doze[6], 'N_EA': doze[7],
    'MRNA': doze[8], 'FC_MRNA': doze[19], 'PRO': doze[9], 'FC': doze[20],
...
team_temp = list(initiate.keys())
team_temp.append(list(para_set_true.keys())[spot])
_team = [' '*8+_+'\n'+' '*2*8+str(para_set_true[_]) for _ in team_temp]
team = '\n'.join(_team)
print(f'Team!\n{team}')

#%%# Simulation [Function]

def simulator(parameter_set, parameter_set_true, parameter_set_mode, parameter_set_rules = None, species = ['N', 'G', 'N_MRNA', 'G_MRNA', 'NP', 'FC', 'FC_MRNA', 'FM', 'FD', 'EI', 'EA'], steps = 10000, cells = 3, time_mini = 0, time_wait = 0, time_maxi = 48, time_unit = 'Hours', time_delta = 0.25, cell_layers = 2, layer_cells = 1, faces = 2*3, seed = None, sup_comp = False, verbose = False):
    
    assert cells == cell_layers * layer_cells, f"We have an ill-defined number of 'cells'!\n\t{cells} ~ {cell_layers * layer_cells}"
    
    assert time_wait == 0 or (time_wait // time_delta > 1 and time_wait % time_delta == 0), f"Caution, the variable 'time_wait' must be either equal to '0' or a positive integer multiple of the variable 'time_delta' greater than '1'!\n\tTime Wait {time_wait}{' '*8}Time Delta {time_delta}"
    ...
    for rule in parameter_set_rules:
        ...
        _state_tor = {'N': 0, 'G': 0, 'N_MRNA': 0, 'G_MRNA': 0, 'NP': 0, 'FC': 0, 'FC_MRNA': 0, 'FM': 0, 'FD': 0, 'EI': 1000, 'EA': 0}
        state_tors = {'state_tor_PRE': _state_tor, 'state_tor_EPI': _state_tor}
        ...
        epoch_halt_tup = (time_wait, time_unit) # Stopping Time!
        ...
        _data_inter_wait = interpolator(simul, species, time_mini, time_wait, time_unit, time_delta, kind = 0, sup_comp = sup_comp, verbose = verbose, err = False, fill_value = 'extrapolate')
        data_inter_wait = (_data_inter_wait[0][0:-int(1/time_delta)], _data_inter_wait[1][:, 0:-int(1/time_delta), :])
        if verbose and time_wait != 0: plotful(data_inter_wait, species, time_unit)
        
        pat = {'FC_MRNA': (para_fun('FC_MRNA'), 0.5), 'FC': (para_fun('FC'), 0.5)}
        initial_pattern = make_initial_pattern(pat, pat_mode, verbose, species = list(stem.assembly['species'].values()), cells = cells, seed = seed)
        
        _state_tor = {'FC_MRNA': 0, 'FC': 0}
        state_tors = {'state_tor_PRE': _state_tor, 'state_tor_EPI': _state_tor}
        state_tor = instate_state_tor(stem, cell_layers, layer_cells, state_tors, pat_mode, initial_pattern, blank = False, state_tor_wait = simul.state_tor[0], state_tor_cast = True)
        
        instate = {'state_tor': state_tor, 'rate_mat': rate_mat}
        simul = BiochemSimulSwift(stem, instate, steps, cells, species, seed, verbose = False)
        jump_diffuse_tor = simul.jump_diffuse_assemble(comm_classes_portrait)
        epoch_halt_tup = (time_maxi-time_wait, time_unit) # Stopping Time!
        if sup_comp or verbose: _a = time.time()
        simul.meth_direct(jump_diffuse_tor, epoch_halt_tup)
        if sup_comp or verbose: _b = time.time()
        if sup_comp or verbose: print(f'Simul\nCells = {simul.cells}\tSteps = {simul.steps}\t', _b-_a)
        
        _data_inter = interpolator(simul, species, time_mini, time_maxi-time_wait, time_unit, time_delta, kind = 0, sup_comp = sup_comp, verbose = verbose, err = False, fill_value = 'extrapolate')
        if time_wait == 0:
            data_inter_0 = np.concatenate((data_inter_wait[0], _data_inter[0]), 0)
            data_inter_1 = np.concatenate((data_inter_wait[1], _data_inter[1]), 1)
        else:
            data_inter_0 = np.concatenate((data_inter_wait[0], _data_inter[0]+data_inter_wait[0][-1]+np.unique(np.diff(data_inter_wait[0]))), 0)
            data_inter_1 = np.concatenate((data_inter_wait[1], _data_inter[1]), 1)
        data_inter = (data_inter_0, data_inter_1)
...
parameter_set_rules = [1] # None
species = ['N', 'G', 'N_MRNA', 'G_MRNA', 'NP', 'FC', 'FC_MRNA', 'FM', 'FD', 'EI', 'EA'] # ['N', 'G', 'NP']
steps = 5000000 if sup_comp else 2500000
...
arguments = (parameter_set, parameter_set_true, parameter_set_mode, parameter_set_rules, species, steps, cells, time_mini, time_wait, time_maxi, time_unit, time_delta, cell_layers, layer_cells, faces, seed, sup_comp, verbose)
...
train_trials = 10
simul_trials = 10
aim_NT = [2/5] if parameter_set_rules == [1] else [1] if parameter_set_rules == [0] else None
aim_G = [3/5] if parameter_set_rules == [1] else [0] if parameter_set_rules == [0] else None
posts = list()
appraisals = list()
...
if inference_prod_activate:
    ...
    for train_trial in range(train_trials):
        # Preparation
        objective_fun_reference = ObjectiveFunPortionRule(trajectory_set, species, time_mini, time_maxi, time_unit, time_delta, simulations_maxi = 10, verbose = False, show = False, rules = parameter_set_rules, aim_NT = aim_NT, aim_G = aim_G, L = _observe)
        ...
        # Synthetic Data
        objective_fun = ObjectiveFunPortionRule(None, species, time_mini, time_maxi, time_unit, time_delta, simulations_maxi = 10, verbose = False, show = False, rules = parameter_set_rules, aim_NT = aim_NT, aim_G = aim_G, L = _observe)
