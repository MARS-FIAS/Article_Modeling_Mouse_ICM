...
uniform = 0 # {0, 1, 5, 10, 25, 50, 75, 100} # Percentage!
reiterate = f'Reiterate_{cellulate[0]}_{cellulate[1]}_Initiate_{_initiate}_Uniform_{uniform}'
...
print(f'Map!\n{_tag}\n\t{mape}')
mape[8] *= initiate['MRNA']/100
mape[9] *= initiate['PRO']/100
span = 5e-31
...
def simulator(parameter_set, parameter_set_true, parameter_set_mode, parameter_set_rules = None, species = ['N', 'G', 'N_MRNA', 'G_MRNA', 'NP', 'FC', 'FC_MRNA', 'FM', 'FD', 'EI', 'EA'], steps = 10000, cells = 3, time_mini = 0, time_maxi = 48, time_unit = 'Hours', time_delta = 0.2, cell_layers = 2, layer_cells = 1, faces = 2*3, uniform = 0, seed = None, sup_comp = False, verbose = False):
    ...
    for rule in parameter_set_rules:
        ...
        pat = {'N_MRNA': (uniform, para_fun('MRNA'), 0.5), 'G_MRNA': (uniform, para_fun('MRNA'), 0.5), 'N': (uniform, para_fun('PRO'), 0.5), 'G': (uniform, para_fun('PRO'), 0.5)}
        pat_mode = 'Uni_Fish_Bind'
...
arguments = (parameter_set, parameter_set_true, parameter_set_mode, parameter_set_rules, species, steps, cells, time_mini, time_maxi, time_unit, time_delta, cell_layers, layer_cells, faces, uniform, seed, sup_comp, verbose)
