...
card = 0 # {1, 5, 10, 25, 50, 75, 100}
initiate = {'N_MRNA': 0, 'G_MRNA': 0, 'N': 0, 'G': 0} # {(200, 0, 200, 0), (175, 25, 175, 25), (150, 50, 150, 50), (125, 75, 125, 75), (100, 100, 100, 100), (75, 125, 75, 125), (50, 150, 50, 150), (25, 175, 25, 175), (0, 200, 0, 200)} # Percentage!
_initiate = '_'.join(map(str, [f'{key}_{value}' for key, value in initiate.items()]))
reiterate = f'Reiterate_Nook_{cellulate[0]}_{cellulate[1]}_Card_{card}_Initiate_{_initiate}'
...
para_set_raw = {
    ...
    'N_MRNA': doze[8], 'G_MRNA': doze[8], 'N': doze[9], 'G': doze[9],
...
#%%# Rand Cell Selection [Function]

def rand_cell_selection(initial_pattern, species_selection = None, card = 0, seed = None, verbose = False):
    if verbose: print(f"Rand Cell Selection!\n{' '*8}Species Selection\n{' '*2*8}{species_selection}\n{' '*8}Cardinality\n{' '*2*8}{card}")
    if species_selection is None or card == 0:
        pass
    else:
        assert initial_pattern[0].keys() == species_selection.keys(), 'Oops! Something went wrong!'
        cells = len(initial_pattern)
        cell_selection = np.sort(np.random.default_rng(seed).choice(a = cells, size = card, replace = False))
        species_lite = list(species_selection.keys())
        lamb = list(species_selection.values())
        prob = [0.5]*len(lamb)
        nook = {_: 0 for _ in species_selection.keys()}
        for cell in range(cells):
            if cell in cell_selection:
                trial = _trial_Fish_Bind(species_lite, lamb, prob, seed, verbose)
                initial_pattern[cell].update(trial)
            else:
                initial_pattern[cell].update(nook)
    if verbose:
        _initial_pattern = '\n'.join(map(str, [initial_pattern[cell] for cell in range(cells)]))
        print(f"{' '*8}Cell Selection\n{' '*2*8}{cell_selection}\nInitial Pattern!\n{_initial_pattern}")
    return initial_pattern

#%%# Simulation [Function]

def simulator(parameter_set, parameter_set_true, parameter_set_mode, parameter_set_rules = None, species = ['N', 'G', 'N_MRNA', 'G_MRNA', 'NP', 'FC', 'FC_MRNA', 'FM', 'FD', 'EI', 'EA'], steps = 10000, cells = 3, time_mini = 0, time_maxi = 48, time_unit = 'Hours', time_delta = 0.2, cell_layers = 2, layer_cells = 1, faces = 2*3, species_selection = None, card = 0, seed = None, sup_comp = False, verbose = False):
    ...
    for rule in parameter_set_rules:
        ...
        initial_pattern = rand_cell_selection(initial_pattern, species_selection, card, seed, verbose)
...
faces = 2*3
species_selection = {'N_MRNA': mape[8]*initiate['N_MRNA']/100, 'G_MRNA': mape[8]*initiate['G_MRNA']/100, 'N': mape[9]*initiate['N']/100, 'G': mape[9]*initiate['G']/100} # pat
# card = 0 # {1, 5, 10, 25, 50}
seed = None # None
# sup_comp = False # True
verbose = True

arguments = (parameter_set, parameter_set_true, parameter_set_mode, parameter_set_rules, species, steps, cells, time_mini, time_maxi, time_unit, time_delta, cell_layers, layer_cells, faces, species_selection, card, seed, sup_comp, verbose)
